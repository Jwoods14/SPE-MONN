...
def pack2D_embed(arr_list):
    N = max([x.shape[0] for x in arr_list])
    M = 768#max([x.shape[1] for x in arr_list])
    a = np.zeros((len(arr_list), N, M))
    for i, arr in enumerate(arr_list):
        n = arr.shape[0]
        m = arr.shape[1]
...
def batch_data_process_embed(data):
    vertex, edge, atom_adj, bond_adj, nbs, sequence, prot_embed = data
    ...
    prot_embed = pack2D_embed(prot_embed)
    ...
    prot_embed = Variable(torch.FloatTensor(prot_embed)).cuda()
    return vertex_mask, vertex, edge, atom_adj, bond_adj, nbs_mask, seq_mask, sequence, prot_embed
...
def data_from_index(data_pack, idx_list):
    ...
    return [fa, fb, anb, bnb, nbs_mat, seq_input, aff_label, pairwise_mask, pairwise_label, pid, cid]
